...
export type SpawnRunner = (command: string, args: string[]) => Promise<{ stdout: string; stderr: string }>;

export function createProcessRunner(): SpawnRunner {
  return (command, args) =>
...
export class RaspController {
  constructor(
    ...
    private readonly run: SpawnRunner = createProcessRunner(),
  ) {}
  ...
  private playerPidFile() {
    return `${this.config.raspActiveRoot}/player.pid`;
  }

  private playerLogFile() {
    return `${this.config.raspActiveRoot}/player.log`;
  }

  private remoteQrPath() {
    return `${this.config.raspActiveRoot}/qr-login.gif`;
  }

  private stopRemotePlayersScript() {
    const rootPattern = regexEscape(this.config.raspActiveRoot);
  ...
  async status(): Promise<RemoteStatus> {
    try {
      const result = await this.run("ssh", [this.config.raspSshTarget, `${this.config.raspActiveRoot}/active/run-show.sh`, "status"]);
  ...
  async showQrCode(url: string) {
    ...
    const activeRunScript = `${this.config.raspActiveRoot}/active/run-show.sh`;
    await writeFile(this.paths.qrImageFile, renderQrGif(url));
    await this.runRemoteShell(`mkdir -p ${shellEscape(this.config.raspActiveRoot)}`);
  ...
  async applyBundle(bundle: BuiltBundle): Promise<LastApplied> {
    const releasesRoot = `${this.config.raspActiveRoot}/releases`;
    const incomingRoot = `${releasesRoot}/.incoming-${bundle.applyId}`;
    const activeRunScript = `${this.config.raspActiveRoot}/active/run-show.sh`;
    const nextReleaseRoot = `${releasesRoot}/${bundle.applyId}`;
    await this.runRemoteShell(`mkdir -p ${shellEscape(releasesRoot)} ${shellEscape(this.config.raspActiveRoot)}`);
    ...
    const pruneCommand = `find ${shellEscape(releasesRoot)} -mindepth 1 -maxdepth 1 -type d -printf '%f\\n' | sort -r | awk 'NR > ${this.config.raspReleasesToKeep} { print }' | while IFS= read -r old_release; do [[ -n "$old_release" ]] || continue; rm -rf ${shellEscape(`${releasesRoot}/`)}"$old_release"; done`;
    const switchResult = await this.runRemoteShell(
      `if [[ -x ${shellEscape(activeRunScript)} ]]; then ${shellEscape(activeRunScript)} stop || true; fi && rm -rf ${shellEscape(nextReleaseRoot)} && mv ${shellEscape(incomingRoot)} ${shellEscape(nextReleaseRoot)} && ln -sfn ${shellEscape(nextReleaseRoot)} ${shellEscape(`${this.config.raspActiveRoot}/active`)} && ${shellEscape(activeRunScript)} start && ${pruneCommand}`,
    );
